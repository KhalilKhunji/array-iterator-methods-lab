...
const nameMapper = (inventor) => {
    return {first: inventor.first, last: inventor.last}
};

inventorNames = inventors.map(nameMapper);
...
const nameReorder = (person) => {
    const name = person.split(',');
    return `${name[1]} ${name[0]}`;
};

firstLast = people.map(nameReorder);
...
isAdultPresent = devs.some((dev) => {return (2023 - dev.year) >= 18});
...
isEveryone19OrOlder = devs.every((dev) => {return (2023-dev.year) >= 19});
...
commentById = comments.find((comment) => {return comment.id === 823423})
...
idx = comments.findIndex((comment) => {return comment.id === 123523})
...
const lifeSum = (sum, span) => {
    return sum + (span.passed - span.year);
};

totalYearsLived = inventors.reduce(lifeSum,0);
...
travelMethodCounts = travelMethods.reduce(methodCount,{})
